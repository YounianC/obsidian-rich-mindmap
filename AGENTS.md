...
`parse` 是对全文的一次线性扫描（带围栏状态机）：顶格标题 `#`–`######` 与嵌套列表（无序 `-`/`*`/`+` 与有序 `1.`/`1)`）都产生节点，共同构成层级。
...
`npm run check:purity` 强制校验（覆盖范围写在 `scripts/check-purity.mjs` 的 `PURE_DIRS` / `PURE_FILES`）。这条边界是整个测试策略的地基：纯函数层有 439 个单测，视图层一个自动化测试都没有。
...
1. 不以换行结尾的文件补上尾换行
2. 松散列表（项之间夹空行）压缩成紧凑列表。**无序与有序是两个不同的列表，有序之间换分隔符（`1.` → `1)`）同样开一个新列表，只有空行两侧属于同一个列表时才压缩**
3. `CRLF` → `LF`
4. 缩进单位按标题各自推断、各自保真。两种情形下改写为每层 2 空格：某个列表块**内部**混用、无法归纳出单一单位；或**同一个标题名下**的多个列表块彼此不一致（统一到第一个能推断出的单位）。不同标题名下用不同单位是保真的。`- ` 宽 2 而 `1. ` 宽 3，所以**有序与无序在同一个块里混排时更容易撞上前一种情形**（`- a` / `  - b` / `1. c` / `   1. d` 会被统一成每层 2 空格）——这是既有规则的新触发面，不是新增的归一化
5. 行内标记按 优先级 → 进度 → 旗帜 排序

**其余任何字节差异都是缺陷。** 具体地：`*` / `+` 列表标记要原样保留（`MindNode.bullet`），有序项的序号与分隔符要原样保留（`MindNode.ordered`）且**序号永不从节点在兄弟里的下标重算**——只有 `tree-ops` 的结构变更才经 `renumber` 改写它，每一个标题行的 `#` 个数与前后空白要原样保留（`HeadingForm.prefix` / `suffix`），frontmatter 结束围栏的尾随空白要原样保留，一切非节点行原样保留（落在 `MindDoc.preamble` 或某个节点的 `continuation` 里）。

**标题行的 `#` 个数永不重算：`serialize` 逐字重放 `heading.prefix`。** 任何形如 `"#".repeat(level)` 的代码都是缺陷 —— 层级归属规则（哪个标题挂在谁名下）与文件字节必须保持解耦，否则调整归属规则就会改写用户的文件。

**空行归属的判据有四个条件，少一个就出缺陷。** 归一化第 2 条（松散列表压缩）只在「前一条目是列表项」**且「它还没有吸收任何 continuation」**且「空行后紧跟列表项」**且「两侧属于同一个列表」**时生效。列表项条目会持续吸收续行，所以「上一个条目是列表项」不等于「上一行是列表项行」；少了第二条，`- a` / 空行 / 散文 / 空行 / `- c` 里第二个空行会被当成松散列表的填充删掉，而它分隔的是段落与列表 —— 那是第六条未获许可的归一化。

第四条（同类守卫）比较的是 `previous.ordered?.delim ?? null`：无序对无序、`1.` 对 `1.` 才算同类，`- a` / 空行 / `1. x` 与 `1. a` / 空行 / `1) x` 都要保留那个空行。它刻意**不**比较无序的标记字符——`- a` / 空行 / `* b` 被压缩是改造前就有的行为，收紧它是另一件事。

语料在 `tests/roundtrip.test.ts` 的「空行归属」注释下面。
...
`removeNode` 的拒绝判据是 `hasHiddenContent(node)`——**continuation 里有没有非空行，而不是「是不是标题节点」**。有序列表**不再**属于「图上不可见的正文」（它现在产生节点），所以只因为吃进有序列表而不可删的标题现在可删了。`## A` 与它名下第一个列表项之间那个空行没有信息，为它禁掉整个标题的删除是过宽的（旗舰示例里 9 个标题有 6 个属于这种情况）。这条判据同样作用于列表项：一个吸收了表格/围栏作为续行的列表项也不可删。
...
- **设置页只有 `display()` 一条路径，不要「顺手」把声明式设置加回来。** Obsidian 1.13 的 `getSettingDefinitions()` 能让设置项被设置面板搜索到，代价是变更钩子 `setControlValue()` 与刷新用的 `update()` 都是 `@since 1.13.0`；`minAppVersion` 停在 1.8.7 时，官方体检的 `obsidianmd/no-unsupported-api` 会把这两处调用报成 **Error**（静态扫描，不管它们其实只在 1.13+ 才被基类调到）。曾经两条路径并存，就是因为这个 Error 才砍掉声明式那条。`obsidian.d.ts` 自己对 `display()` 的说明是「Only implement display() as a fallback for plugins that need to support Obsidian versions older than 1.13.0」——所以体检报的那条 `display` deprecated 是预期内的，**不是要修的东西**。真要用声明式，得连 `manifest.json` 的 `minAppVersion` 与 `versions.json` 一起抬到 1.13.0，那是「放弃 1.13 以下用户」的产品决策，不是重构。
...
备注是**紧跟节点行的连续引用行**（`> …`），解析期由 `src/model/note.ts` 的 `splitNote` 从 `continuation` 里摘进 `MindNode.note`，序列化期由 `serializeNote` 写回。选引用块而不是 HTML 注释或 frontmatter 映射表，核心理由是 **`>` 前缀天然是逃逸字符**：备注正文里的 `- x` / `# x` / 代码围栏被写成 `> - x` / `> # x` / `> ``` ` 之后，三条节点行正则（顶格 `^#`、`^[ \t]*[-*+][ \t]+`、`^[ \t]*` 后接围栏字符）全都不匹配，用户写什么都不可能长出假节点或打开假围栏。回归测试在 `tests/note.test.ts` 的「备注正文不改变导图结构」里，用 fast-check 生成敌意行。

**`Note.raw` 不能省。** 用户写的可能是 `>备注`（无空格）、`>  备注`（两个空格）、tab 缩进。只存解码后的 `text`、写回时统一渲染成 `> `，就是第六条未获许可的写回归一化。解析期把原始行整条留住，只有用户真正编辑过备注才置 `raw = null`，那时才由 `serialize` 按节点当前的缩进重新生成。

**`moveNode` 必须清掉移动子树内所有 `note.raw`**：深度变了旧缩进不再对，留着会让备注行掉出列表外（仍能解析回同一个节点，但 Obsidian 自己的阅读视图里位置是错的）。

**根节点永不带备注**，与第 8 条对标记的规则一致：`setNote` 对根 id 是 no-op，`parse` 对称地不对根做 `splitNote`，工具栏按钮在选中根节点时禁用。

**备注不在 `continuation` 里，所以 `hasHiddenContent` 看不见它**——加备注不会让节点变成不可删、不可拖。这是刻意的：备注在导图上是可见的（角标 + 气泡），不属于「图上不可见的正文」。不要把备注行留在 `continuation` 里「省事」，那会静默禁用删除与拖拽两个按钮。
...
**release notes 取自带注释 tag 的说明**（`git tag -a`），所以 tag 消息就是要发给用户看的文案。workflow 里有一步 `git fetch --force origin refs/tags/<tag>:refs/tags/<tag>` 是必需的：`actions/checkout` 只把 tag 取成指向 commit 的轻量 ref，不补这一步 `git cat-file -t` 永远返回 `commit`，notes 会静默退化成自动生成的 changelog 链接（0.2.0 / 0.2.1 就是这么发出去的）。
