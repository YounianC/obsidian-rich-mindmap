...
未闭合或写错的标记（比如落单的 `**`、`*`）会原样按文字显示，不会报错也不会丢字符。`#标签`、图片和原始 HTML **不会**被渲染——节点文字原样保留（这样 `#标签` 在 vault 其他地方仍然有效），只是导图上不会把它变成链接或嵌入。工具栏的加粗/斜体/删除线/链接按钮写的就是这套语法。
...
- 纯函数层（解析、序列化、标记、折叠状态、树操作、布局、相机）有 232 个自动化测试，含 Markdown 往返属性测试。
