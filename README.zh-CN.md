...
把 [`examples/conference-talk.md`](examples/conference-talk.md) 拷进你的 vault 用导图视图打开即可。那份文件把所有特性都用到了：`##` / `###` 分节构成外层层级、7 档优先级、7 种旗帜颜色、覆盖每一档的进度值、嵌套列表、双链、行内格式，一个**不是**标记的括号组，一个记在 `mindmap-collapsed` 里的折叠标题，以及被插件隐形携带、不显示的 frontmatter 其他键、散文段落和有序列表。
